...
checkable(Atom) :- Atom = just_left_of(A, B).
checkable(Atom) :- Atom = left_of(A, B).

check_claim(Atom) :- Atom = just_left_of(A, B), check_just_left_of(A, B).
check_claim(Atom) :- Atom = left_of(A, B), check_left_of(A, B).
